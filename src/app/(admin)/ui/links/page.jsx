...
const ColoredLinks = () => {
  return <ComponentContainerCard title='Colored links' description={<>You can use the <code>.link-*</code> classes to colorize links. Unlike the <Link href="/ui/utilities"><code>.text-*</code> classes</Link>, these classes have a <code>:hover</code> and <code>:focus</code> state. Some of the link styles use a relatively light foreground color, and should only be used on a dark background in order to have sufficient contrast.</>}>
      {colorVariants.slice(0, 6).map((item, idx) => <p key={idx}><Link href="" className={`link-${item}`}>{item.charAt(0).toUpperCase() + item.slice(1)} link</Link></p>)}
...
const LinkUtilities = () => {
  return <ComponentContainerCard title='Link utilities' description={<><Link href="/ui/utilities">Colored link helpers</Link> have been updated to pair with our link utilities. Use the new utilities to modify the link opacity, underline opacity, and underline offset.</>}>
      {colorVariants.slice(0, 6).map((item, idx) => <p key={idx}><Link href="" className={`link-${item} text-decoration-underline link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover`}>{item.charAt(0).toUpperCase() + item.slice(1)} link</Link></p>)}
...
const LinkOpacity = () => {
  return <ComponentContainerCard title='Link opacity' description={<>Change the alpha opacity of the link <code>rgba()</code> color value with utilities. Please be aware that changes to a color’s opacity can lead to links with <em>insufficient</em> contrast.</>}>
...
const LinkHoverOpacity = () => {
  return <ComponentContainerCard title='Link hover opacity' description={<>You can even change the opacity level on hover.</>}>
...
const UnderlineColor = () => {
  return <ComponentContainerCard title='Underline color' description={<>Change the underline’s color independent of the link text color.</>}>
...
const UnderlineOpacity = () => {
  return <ComponentContainerCard title='Underline opacity' description={<>Change the underline’s opacity. Requires adding <code>.link-underline</code> to first set an <code>rgba()</code> color we use to then modify the alpha opacity.</>}>
...
const HoverVariants = () => {
  return <ComponentContainerCard title='Hover variants' description={<>Just like the <code>.link-opacity-*-hover</code> utilities, <code>.link-offset</code> and <code>.link-underline-opacity</code> utilities include <code>:hover</code> variants by default. Mix and match to create unique link styles.</>}>
