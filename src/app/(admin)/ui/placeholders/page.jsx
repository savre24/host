...
const DefaultPlaceholders = () => {
  return <ComponentContainerCard title='Placeholders' description={<>In the example below, we take a typical card component and recreate it with placeholders applied to create a “loading card”. Size and proportions are the same between the two.</>}>
      <Row>
        <Col md={6}>
...
const ColorPlaceholders = () => {
  return <ComponentContainerCard title='Color' description={<> By default, the <code>placeholder</code> uses <code>currentColor</code>. This can be overriden with a custom color or utility class.</>}>
...
const WidthPlaceholders = () => {
  return <ComponentContainerCard title='Width' description={<>You can change the <code>width</code> through grid column classes, width utilities, or inline styles.</>}>
      <span className="placeholder col-6" />
...
const SizingPlaceholders = () => {
  return <ComponentContainerCard title='Sizing' description={<> The size of <code>.placeholder</code>s are based on the typographic style of the parent element. Customize them with sizing modifiers: <code>.placeholder-lg</code>, <code>.placeholder-sm</code>, or <code>.placeholder-xs</code>.</>}>
...
const WorksPlaceholder = () => {
  return <ComponentContainerCard title='How it works' description={<>Create placeholders with the <code>.placeholder</code> class and a grid column class (e.g., <code>.col-6</code>) to set the <code>width</code>. They can replace the text inside an element or as be added as a modifier class to an existing component.</>}>
      <p aria-hidden="true">
        <span className="placeholder col-6" />
      </p>
...
const AnimationPlaceholder = () => {
  return <ComponentContainerCard title='Animation' description={<>Animate placehodlers with <code>.placeholder-glow</code> or <code>.placeholder-wave</code> to better convey the perception of something being <em>actively</em> loaded.</>}>
