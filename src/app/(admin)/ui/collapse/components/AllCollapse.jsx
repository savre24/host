...
const DefaultCollapse = () => {
  ...
  return <ComponentContainerCard title='Collapse' description={<> Bootstrap&apos;s collapse provides the way to toggle the visibility of any content or element.
...
const CollapseHorizontal = () => {
  ...
  return <ComponentContainerCard title='Collapse Horizontal' description={<>The collapse plugin also supports horizontal collapsing. Add the <code>.collapse-horizontal</code> modifier class to transition the <code>width</code> instead of <code>height</code> and set a <code>width</code> on the immediate child element.</>}>
      <p>
...
const MultipleCollapse = () => {
  ...
  return <ComponentContainerCard title='Multiple Targets' description={<> Multiple <code>&lt;button&gt;</code> or <code>&lt;a&gt;</code> can show and hide an element if
