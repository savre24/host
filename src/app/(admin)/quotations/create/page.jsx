import { getClients } from '@/app/actions/client';
import { getActiveProducts } from '@/app/actions/product';
import { getTaxSetting } from '@/app/actions/settings';
import ComponentContainerCard from '@/components/ComponentContainerCard';
import PageTitle from '@/components/PageTitle';
import Link from 'next/link';
import { Col, Row } from 'react-bootstrap';
import QuotationForm from './QuotationForm';
import IconifyIcon from '@/components/wrappers/IconifyIcon';

export const metadata = {
  title: 'Create Quotation',
};

const CreateQuotationPage = async () => {
  // Fetch all clients to populate the dropdown
  const { data: clients, error } = await getClients();
  // Fetch services/products
  const { data: products } = await getActiveProducts();
  
  // Fetch default tax setting
  const { data: taxSetting } = await getTaxSetting();
  const defaultTaxRate = taxSetting?.isEnabled ? taxSetting.percentage : 0;

  // Generate a random quotation number like INV-2026-XXXXX
  const randomStr = Math.random().toString(36).substring(2, 7).toUpperCase();
  const autoQuotationNumber = `INV-${new Date().getFullYear()}-${randomStr}`;

  return (
    <>
      <PageTitle title="Create New Quotation" subTitle="Billing" />
      <Row className="mb-3">
        <Col>
          <Link href="/quotations" className="btn btn-outline-secondary">
            <IconifyIcon icon="tabler:arrow-left" width={18} height={18} className="me-1" /> Back to Quotations
          </Link>
        </Col>
      </Row>
      
      <Row>
        <Col xs={12}>
          <ComponentContainerCard title="Quotation Details" description="Fill out the details below to generate a new quotation.">
            {error ? (
              <div className="alert alert-danger">Failed to load clients. Please try again.</div>
            ) : (
              <Col lg={10} className="mx-auto">
                <QuotationForm 
                  clients={clients || []} 
                  products={products || []} 
                  defaultQuotationNumber={autoQuotationNumber} 
                  defaultTaxRate={defaultTaxRate}
                />
              </Col>
            )}
          </ComponentContainerCard>
        </Col>
      </Row>
    </>
  );
};

export default CreateQuotationPage;
