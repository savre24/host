  ...
  return <Card className="h-100">
      ...
      <CardBody className="p-3">
        ...
        <ul className="d-flex flex-column gap-2 list-unstyled fs-15">
          {features.map((item, idx) => <li key={idx}>
      ...
      {popular && <span className="position-absolute top-0 translate-middle start-50 bg-warning-subtle px-3 py-1 text-warning rounded-pill fw-semibold">
          Most Popular
        </span>}
    </Card>;
};
      ...
      <Row className="justify-content-center">
        <Col xxl={9}>
          ...
          <Row className="mt-sm-4 align-items-end justify-content-center my-3">
            {pricingTwoData.map((item, idx) => <Col lg={4} key={idx}>
                  <PricingCard {...item} />
                </Col>)}
          </Row>
        </Col>
