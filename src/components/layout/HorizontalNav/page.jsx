...
const MenuItemWithChildren = ({
  ...
  console.log(open);
  return <Tag className={`${className} dropdown ${activeMenuItems.includes(item.key) ? 'active' : ''}`}>
      <Link href={''} className={` ${level > 1 ? 'dropdown-item' : 'nav-link'} drop-arrow-none`} id="topnav-dashboards" data-bs-toggle="dropdown" onClick={toggleMenuItem} role="button" aria-haspopup="true" aria-expanded="false">
      ...
      <ul className={`dropdown-menu ${open && 'show'}`} data-bs-popper="static" aria-labelledby="topnav-dashboards">
        {(item.children || []).map((child, idx) => {
        return <Fragment key={child.key + idx}>
              {child.children ? <MenuItemWithChildren item={child} tag={'li'} linkClassName={clsx('nav-link', getActiveClass(child))} activeMenuItems={activeMenuItems} className="dropdown" level={level + 1} subMenuClassName="sub-menu" toggleMenu={toggleMenu} /> : <MenuItem level={level + 1} item={child} linkClassName={clsx('dropdown-item', getActiveClass(child))} />}
            </Fragment>;
      })}
      </ul>
    </Tag>;
              ...
              <div id="topnav-menu-content">
                <ul className="navbar-nav dropdown-hover">
                  ...
                  return <Fragment key={idx}>
                        {item.children ? <MenuItemWithChildren item={item} toggleMenu={toggleMenu} className="nav-item" level={1} linkClassName={clsx('nav-link', getActiveClass(item))} subMenuClassName="sub-menu" tag="li" activeMenuItems={activeMenuItems} /> : <MenuItem item={item} level={1} linkClassName={clsx('dropdown-item nav-link', getActiveClass(item))} className="nav-item" />}
                      </Fragment>;
                })}
                </ul>
              </div>
